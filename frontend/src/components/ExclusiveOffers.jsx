  ...
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-10 pb-30'>
        <div className='flex flex-col md:flex-row w-full justify-between items-center'>
            <Title title='Exclusive Offers' subTitle='Take advantage of limited time offers and special packages to enhance your stay and create unforgettable memories' align={'left'}  />
            <button className='flex items-center gap-2 font-medium cursor-pointer group max-md:mt-12'>View All Offers
            ...
            {exclusiveOffers.map((item)=>(
                <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center ' style={{backgroundImage:`url(${item.image})`}}>
                  ...
                  <button className='flex items-center gap-2 font-medium cursor-pointer  mt-4 mb-5'>
                    View Offers 
                <img src={assets.arrowIcon} alt="arrow-icon"  className='invert transition-all group-hover:translate-x-1'/>
